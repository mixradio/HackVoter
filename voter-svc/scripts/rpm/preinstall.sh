...
then
  ...
  /bin/echo "Voter is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Voter user for the Voter service" $APP_NAME 2> /dev/null || :
fi
